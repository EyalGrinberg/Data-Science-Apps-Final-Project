...
```{r}
if(F){
  install.packages(c("DataExplorer","SmartEDA","explore","tidytext","dataMaid"))
}
```
...
```{r}
cakes_mean_by_nut <- cakes_nutrients %>% group_by(name) %>% summarize(mean_amount = mean(amount))
choco_mean_by_nut <- choco_nutrients %>% group_by(name) %>% summarize(mean_amount = mean(amount))
popcorn_mean_by_nut <- popcorn_nutrients %>% group_by(name) %>% summarize(mean_amount = mean(amount))
candy_mean_by_nut <- candy_nutrients %>% group_by(name) %>% summarize(mean_amount = mean(amount))
chips_mean_by_nut <- chips_nutrients %>% group_by(name) %>% summarize(mean_amount = mean(amount))
cookies_mean_by_nut <- cookies_nutrients %>% group_by(name) %>% summarize(mean_amount = mean(amount))
test_mean_by_nut <- test_nutrients %>% group_by(name) %>% summarize(mean_amount = mean(amount))
```

```{r}
cakes_num_nutrients <- cakes_nutrients %>% group_by(idx) %>% summarize(num_nutrients = length(name))
choco_num_nutrients <- choco_nutrients %>% group_by(idx) %>% summarize(num_nutrients = length(name))
popcorn_num_nutrients <- popcorn_nutrients %>% group_by(idx) %>% summarize(num_nutrients = length(name))
candy_num_nutrients <- candy_nutrients %>% group_by(idx) %>% summarize(num_nutrients = length(name))
chips_num_nutrients <- chips_nutrients %>% group_by(idx) %>% summarize(num_nutrients = length(name))
cookies_num_nutrients <- cookies_nutrients %>% group_by(idx) %>% summarize(num_nutrients = length(name))
test_num_nutrients <- test_nutrients %>% group_by(idx) %>% summarize(num_nutrients = length(name))
```

```{r}
plots_func_num_nutriens_by_cat <- function(x, tilte_name) {
  hist(x$num_nutrients, main = tilte_name)
}
```
